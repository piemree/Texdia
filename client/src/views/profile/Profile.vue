...
export default {
  ...
  methods: {
    ...
    uploadImage(event) {
      const URLL = "http://localhost:5000/api/upload";

      let reader = new FileReader();

      reader.addEventListener("load",() => {
        this.$store.dispatch("updateProfilePicture",reader.result)
      /*   console.log(reader.result)
        axios.post(URLL, {picture:reader.result}).then((result) => {
  ...
  <div class="profile">
    ...
    <div v-else class="profile-header">
      <div class="profile-picture">
        <img ref="image" :src="userProfile.user.picture" />
      </div>
